        ...
        public void WriteResultPane(FATabStripItemCollection tabPages)
        {
            ...
            foreach (FATabStripItem tp in tabPages)
            {
                ...
                else if (tp is LRResultTabPage)
                {
                    ...
                    if (lrTp.IsAnovaTableVisible) seriesXmlWriter.WriteElementString("View", "LRAnova");
                    if (lrTp.IsCoefficientTableVisible) seriesXmlWriter.WriteElementString("View", "LRCoefficients");
                    if (lrTp.IsDataTableVisible) seriesXmlWriter.WriteElementString("View", "LRActualPredictedResidual");
                    if (lrTp.IsForcastedTableVisible) seriesXmlWriter.WriteElementString("View", "Forcasted");
                    if (lrTp.IsResidualGraphVisible) seriesXmlWriter.WriteElementString("View", "LRResidualGraph");
                    if (lrTp.IsResidualVsPredictedGraphVisible) seriesXmlWriter.WriteElementString("View", "LRResidualVsPredictedGraph");
                    if (lrTp.IsNormalProbabilityPlotVisible) seriesXmlWriter.WriteElementString("View", "LRNormalProbabilityPlot");
                    seriesXmlWriter.WriteFullEndElement();
                }
